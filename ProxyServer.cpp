	...
	while (true) {
		...
		for (size_t i = 0; i < this->conns_in.size(); i++) {
			...
			if ((conn_in.revents & (POLLHUP | POLLERR)) || (conn_out.revents & (POLLHUP | POLLERR))) {
				std::cout << "[" << conn_in.fd << "]: Hang up" << std::endl;
				continue;
			}
		...
		for (; conn_in != this->conns_in.rend() && conn_out != this->conns_out.rend(); conn_in++, conn_out++) {
			if ((conn_in->revents & (POLLHUP | POLLERR)) || (conn_out->revents & (POLLHUP | POLLERR))) {
				close(conn_in->fd);
				close(conn_out->fd);
				this->conns_in.erase(conn_in.base());
				this->conns_out.erase(conn_out.base());
			}
		}
	}
	return 0;
...
int ProxyServer::accept_conn() {
	...
	if (in_fd < 0) {
		std::cout << "[DEBUG] ERROR: accept " << in_fd << std::endl;
		return -1;
	}
	std::cout << "[" << in_fd << "]: accepted connection from " << str << std::endl;
